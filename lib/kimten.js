...
export function Kimten(config) {
  ...
  const toolsInstruction = toolNames.length > 0
    ? `\n\nTool policy: You can use these tools when needed for accurate answers: ${toolNames.join(', ')}. Do not fabricate tool results.`
    : '';
  const boxSchemaLine = box
    ? `Return ONLY a valid JSON object that exactly matches this schema (field names/types required): ${describeZodSchema(box)}`
    : '';

  const agent = new ToolLoopAgent({
    model: brain,
    instructions: `${personality}${toolsInstruction}`,
    tools,
  ...
  async function play(input, context = null) {
    ...
    const effectiveUserInput = serializedContext
      ? `Context (JSON):\n${serializedContext}\n\nUser message:\n${input}`
      : input;
    const effectiveInput = boxSchemaLine
      ? `${boxSchemaLine}\n\n${effectiveUserInput}`
      : effectiveUserInput;
