...
export function Kimten(config) {
  ...
  const TOOL_PROMPT_LINE = 'Use provided tools when needed; never invent tool outputs.';
  const BOX_PROMPT_LINE = 'Return only valid JSON that exactly matches the configured output schema.';
  ...
  async function play(input, context = null) {
    ...
    if (Object.keys(tools).length > 0) {
      promptAddons.push(TOOL_PROMPT_LINE);
    }
    if (box) {
      promptAddons.push(BOX_PROMPT_LINE);
    }

    const effectiveUserMessageBody = serializedContext
      ? `Context (JSON):\n${serializedContext}\n\nUser message:\n${input}`
      : input;
    const effectiveInput = promptAddons.length > 0
      ? `Runtime instructions:\n${promptAddons.join('\n')}\n\n${effectiveUserMessageBody}`
      : effectiveUserMessageBody;
