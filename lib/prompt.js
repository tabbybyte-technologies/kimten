...
export function buildMessagesForAgent(memoryMessages, effectiveInput, rawInput, replacementContent = effectiveInput) {
  const lastMessage = memoryMessages[memoryMessages.length - 1];
  const shouldReplaceLastUserMessage = replacementContent !== rawInput;

  if (!shouldReplaceLastUserMessage || lastMessage?.role !== 'user') {
    return memoryMessages;
  }

  return [...memoryMessages.slice(0, -1), { ...lastMessage, content: replacementContent }];
}
