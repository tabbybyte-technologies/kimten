...
export function Kimten(config) {
  ...
  const BOX_ENFORCEMENT_INSTRUCTION =
    [
      'Output contract:',
      '- Return only valid JSON that conforms exactly to the configured output schema.',
      '- Do not include markdown, code fences, or extra commentary outside the JSON value.',
    ].join('\n');
  const TOOLS_ENFORCEMENT_INSTRUCTION =
    [
      'Tool-use policy:',
      '- Use provided tools when they are needed for accurate, grounded answers.',
      '- Do not invent tool outputs.',
      '- If a tool is unavailable or fails, continue with best-effort reasoning and state limitations clearly.',
    ].join('\n');

  const instructionAddons = [];
  if (Object.keys(tools).length > 0) {
    instructionAddons.push(TOOLS_ENFORCEMENT_INSTRUCTION);
  }
  if (box) {
    instructionAddons.push(BOX_ENFORCEMENT_INSTRUCTION);
  }
  const instructions = instructionAddons.length > 0
    ? `${personality}\n\n${instructionAddons.join('\n\n')}`
    : personality;
